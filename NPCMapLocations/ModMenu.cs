	...
	public class ModMenu : IClickableMenu
	{
		...
		private void SetScrollBarToCurrentIndex()
		{
			if (options.Any())
			{
				this.scrollBar.bounds.Y =
					this.scrollBarRunner.Height / Math.Max(1, this.options.Count - 7 + 1) * this.currentItemIndex +
					this.upArrow.bounds.Bottom + Game1.pixelZoom;
		...
		public override void leftClickHeld(int x, int y)
		{
			...
			else
			{
				...
				this.options[this.currentItemIndex + this.optionsSlotHeld].leftClickHeld(
					x - this.optionSlots[this.optionsSlotHeld].bounds.X, y - this.optionSlots[this.optionsSlotHeld].bounds.Y);
				return;
			}
		}
		...
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			...
			if (this.downArrow.containsPoint(x, y) &&
			    this.currentItemIndex < Math.Max(0, this.options.Count<OptionsElement>() - 7))
			{
				this.DownArrowPressed();
			...
			for (int i = 0; i < this.optionSlots.Count<ClickableComponent>(); i++)
			{
				if (this.optionSlots[i].bounds.Contains(x, y) &&
				    this.currentItemIndex + i < this.options.Count<OptionsElement>() && this.options[this.currentItemIndex + i]
					    .bounds.Contains(x - this.optionSlots[i].bounds.X, y - this.optionSlots[i].bounds.Y))
				{
					this.options[this.currentItemIndex + i]
						.receiveLeftClick(x - this.optionSlots[i].bounds.X, y - this.optionSlots[i].bounds.Y);
		...
		public override void draw(SpriteBatch b)
		{
			...
			Game1.drawDialogueBox(this.mapX - Game1.pixelZoom * 8, this.mapY - Game1.pixelZoom * 24,
				(this.map.Bounds.Width + 16) * Game1.pixelZoom, 212 * Game1.pixelZoom, false, true, null, false);
			b.Draw(this.map, new Vector2((float) this.mapX, (float) this.mapY), new Rectangle?(new Rectangle(0, 0, 300, 180)),
				Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.86f);
			b.Draw(Game1.fadeToBlackRect, Game1.graphics.GraphicsDevice.Viewport.Bounds, Color.Black * 0.4f);
			Game1.drawDialogueBox(this.xPositionOnScreen, this.yPositionOnScreen, this.width, this.height, false, true, null,
				false);
			...
			if (!GameMenu.forcePreventClose)
			{
				...
				{
					IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(403, 383, 6, 6), this.scrollBarRunner.X,
					...
					if (this.currentItemIndex >= 0 && this.currentItemIndex + i < this.options.Count<OptionsElement>())
					{
						if (options[this.currentItemIndex + i] is MapModButton)
						{
							...
							IClickableMenu.drawTextureBox(b, Game1.menuTexture, new Rectangle(0, 256, 60, 60), bounds.X, bounds.Y,
								bounds.Width, bounds.Height, Color.White * (options[this.currentItemIndex + i].greyedOut ? 0.33f : 1f),
								1f, false);
						}
			...
			{
				b.Draw(Game1.mouseCursors, new Vector2((float) Game1.getOldMouseX(), (float) Game1.getOldMouseY()),
					...
					(float) Game1.pixelZoom + Game1.dialogueButtonScale / 150f, SpriteEffects.None, 1f);
			}
		}
	}
	...
	internal class MapModSlider : OptionsElement
	{
		...
		public override void leftClickHeld(int x, int y)
		{
			...
			this.value = ((x >= this.bounds.X)
				? ((x <= this.bounds.Right - 10 * Game1.pixelZoom)
					? ((int) ((double) ((float) (x - this.bounds.X) / (float) (this.bounds.Width - 10 * Game1.pixelZoom)) *
					          (double) this.sliderMaxValue))
					: this.sliderMaxValue)
				: 0);
		...
		public override void draw(SpriteBatch b, int slotX, int slotY)
		{
			...
			IClickableMenu.drawTextureBox(b, Game1.mouseCursors, OptionsSlider.sliderBGSource, slotX + this.bounds.X,
				slotY + this.bounds.Y, this.bounds.Width, this.bounds.Height, Color.White, (float) Game1.pixelZoom, false);
			b.Draw(Game1.mouseCursors,
				new Vector2(
					(float) (slotX + this.bounds.X) + (float) (this.bounds.Width - 10 * Game1.pixelZoom) *
					((float) this.value / (float) this.sliderMaxValue), (float) (slotY + this.bounds.Y)),
				new Rectangle?(OptionsSlider.sliderButtonRect), Color.White, 0f, Vector2.Zero, (float) Game1.pixelZoom,
