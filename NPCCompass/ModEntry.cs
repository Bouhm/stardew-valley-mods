...
namespace NPCCompass
{
    ...
    public class ModEntry : Mod
    {
        ...
        {
            // Triangle such that hypotenuse is the line from player to npc
        ...
        private Vector2 GetLocatorPosition(double angle, int quadrant)
        {
            ...
            switch (quadrant)
            {
                ...
                case 3:
                    ...
                    return new Vector2(Game1.viewport.Width - Game1.tileSize, y);
                case 4:
                    x += Game1.viewport.Width/2 - (Game1.viewport.Height/2 * (float)Math.Tan(3 * MathHelper.PiOver2 - angle));
                    return new Vector2(x, Game1.viewport.Height - Game1.tileSize);
                default:
                    return Vector2.Zero;
            }
        }
        ...
        private void UpdateLocators()
        {
            ...
            {
                if ((npc.Schedule == null && !npc.isMarried()) || npc.currentLocation == null) { continue; }
                //monitor.Log(npc.currentLocation.name);
                ...
                double angle = GetPlayerToNPCAngle(new Vector2(Game1.viewport.X + Game1.viewport.Width/2, Game1.viewport.Y + Game1.viewport.Height/2), npcPos); // playerPos, npcPos);
        ...
        private void GraphicsEvents_OnPreRenderHudEvent(object sender, EventArgs e)
        {
            ...
            {
                Game1.spriteBatch.Draw(locator.Marker, new Vector2(locator.X - 32, locator.Y - 30), new Rectangle?(new Rectangle(0, constants.MarkerCrop[locator.Name], 16, 15)), Color.White, 0f, Vector2.Zero, Game1.pixelZoom, SpriteEffects.None, 1f);
            }
        }
        ...
        private static void ShowDebugInfo()
        {
            ...
            DrawText("Angle (rad): " + Math.Round(ModEntry._angle, 2), new Vector2(Game1.tileSize / 4, Game1.tileSize / 4));
            DrawText("Locator: (" + (int)ModEntry._locatorPos.X + ", " + (int)ModEntry._locatorPos.Y + ")", new Vector2(Game1.tileSize / 4, Game1.tileSize * 3 / 4 + 8));
            DrawText("Player: (" + (int)ModEntry._playerPos.X + ", " + (int)ModEntry._playerPos.Y + ")", new Vector2(Game1.tileSize / 4, Game1.tileSize * 5 / 4 + 8 * 2));
            DrawText("NPC: (" + (int)ModEntry._npcPos.X + ", " + (int)ModEntry._npcPos.Y + ")", new Vector2(Game1.tileSize / 4, Game1.tileSize * 7 / 4 + 8 * 3));
            DrawText("Viewport: (" + (int)ModEntry._viewportPos.X + ", " + (int)ModEntry._viewportPos.Y + ")", new Vector2(Game1.tileSize / 4, Game1.tileSize * 9 / 4 + 8 * 4));
            DrawText(Game1.viewport.Width + "x" + Game1.viewport.Height, new Vector2(Game1.tileSize / 4, Game1.tileSize * 11 / 4 + 8 * 5));
        }
        ...
        private void GraphicsEvents_OnPostRenderEvent(object sender, EventArgs e)
        {
            ...
            foreach (NPC npc in Utility.getAllCharacters())
            {
                ...
                float viewportX = Game1.viewport.Width/2; // Game1.player.position.X - Game1.viewport.X;
                float viewportY = Game1.viewport.Height/2; // Game1.player.position.Y - Game1.viewport.Y;
